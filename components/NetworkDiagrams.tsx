...
export function HeroMeshDiagram() {
  return (
    <DiagramFrame
      title="Network view"
      caption="DHT means Distributed Hash Table. In VX6, any known live peer can onboard a new node. Discovery then expands via signed records and DHT lookups for node and service keys."
    >
      <svg viewBox="0 0 760 490" role="img" aria-label="VX6 peer network overview">
        ...
        <text x="58" y="320" className="diagram-text">Any live peer is enough.</text>
...
export function LocalhostBridgeDiagram() {
  return (
    <DiagramFrame
      ...
      caption="Example: host keeps app on localhost:2000 and client maps it to localhost:1000. Users operate on local ports while VX6 handles peer routing in the background."
    >
      <svg viewBox="0 0 880 360" role="img" aria-label="Localhost to localhost service sharing">
        ...
        <text x="92" y="162" className="diagram-chip-text">localhost:2000 (host)</text>
        ...
        <text x="676" y="162" className="diagram-chip-text">localhost:1000 (client)</text>
        ...
        <text x="440" y="294" className="diagram-chip-text" textAnchor="middle">
          Service in: localhost:2000. Service out: localhost:1000 on client.
        </text>
        <text x="440" y="316" className="diagram-chip-text" textAnchor="middle">
          No direct public app port exposure required.
        </text>
      </svg>
    </DiagramFrame>
  );
...
export function TorVsVx6Diagram() {
  return (
    <DiagramFrame
      ...
      caption="VX6 is optimized for private service routing and localhost workflows, not public web-exit anonymity. Its focus is hidden aliases, local app access, and peer-operated infrastructure."
    >
...
export function HiddenFlowDiagram() {
  return (
    <DiagramFrame
      title="Hidden service flow (sequence + crypto)"
      caption="The flow is split into five phases: descriptor publish, descriptor lookup, setup signaling, dual-leg rendezvous build, and encrypted data forwarding to the owner’s localhost service."
    >
      <svg viewBox="0 0 1340 820" role="img" aria-label="VX6 hidden service flow sequence and cryptography">
        <defs>
          <marker
            id="hidden-arrow"
            markerWidth="10"
        ...
        <rect x="36" y="74" width="1268" height="72" rx="16" className="diagram-panel diagram-panel-accent" />
        <text x="670" y="104" className="diagram-label" textAnchor="middle">Phase 1: owner publishes encrypted descriptor</text>
        <text x="670" y="130" className="diagram-text" textAnchor="middle">
          Signed descriptor {'->'} DHT replicas (includes intro set, guard set, expiry, version)
        </text>

        <rect x="36" y="170" width="1268" height="72" rx="16" className="diagram-panel" />
        <text x="670" y="200" className="diagram-label" textAnchor="middle">Phase 2: client resolves alias and validates consensus</text>
        <text x="670" y="226" className="diagram-text" textAnchor="middle">
          Client fetches from multiple DHT holders {'->'} checks signature, version, expiry, and match threshold
        </text>

        <rect x="56" y="280" width="190" height="384" rx="16" className="diagram-chip" />
        <text x="151" y="310" className="diagram-chip-text" textAnchor="middle">Client (C)</text>
        <text x="151" y="336" className="diagram-text" textAnchor="middle">chooses intro + X</text>
        <line x1="151" y1="356" x2="151" y2="650" className="diagram-path-soft" />

        <rect x="304" y="280" width="210" height="384" rx="16" className="diagram-chip diagram-chip-soft" />
        <text x="409" y="310" className="diagram-chip-text" textAnchor="middle">DHT + Intro set</text>
        <text x="409" y="336" className="diagram-text" textAnchor="middle">i1 / i2 / i3 active</text>
        <line x1="409" y1="356" x2="409" y2="650" className="diagram-path-soft" />

        <rect x="572" y="280" width="210" height="384" rx="16" className="diagram-chip diagram-chip-soft" />
        <text x="677" y="310" className="diagram-chip-text" textAnchor="middle">Owner guards</text>
        <text x="677" y="336" className="diagram-text" textAnchor="middle">g1 / g2 control path</text>
        <line x1="677" y1="356" x2="677" y2="650" className="diagram-path-soft" />

        <rect x="840" y="280" width="210" height="384" rx="16" className="diagram-chip diagram-chip-soft" />
        <text x="945" y="310" className="diagram-chip-text" textAnchor="middle">Rendezvous (X)</text>
        <text x="945" y="336" className="diagram-text" textAnchor="middle">joins both tunnel legs</text>
        <line x1="945" y1="356" x2="945" y2="650" className="diagram-path-soft" />

        <rect x="1108" y="280" width="190" height="384" rx="16" className="diagram-service" />
        <text x="1203" y="310" className="diagram-chip-text" textAnchor="middle">Owner service</text>
        <text x="1203" y="336" className="diagram-text" textAnchor="middle">localhost only</text>
        <line x1="1203" y1="356" x2="1203" y2="650" className="diagram-path-soft" />

        <text x="72" y="390" className="diagram-note">Phase 3 setup signaling</text>
        <path d="M166 412H392" className="diagram-path-soft" markerEnd="url(#hidden-arrow)" />
        <path d="M424 430H662" className="diagram-path-soft" markerEnd="url(#hidden-arrow)" />
        <text x="330" y="406" className="diagram-text">C {'->'} intro</text>
        <text x="544" y="424" className="diagram-text">intro {'->'} guards</text>

        <text x="72" y="504" className="diagram-note">Phase 4 dual-leg rendezvous build</text>
        <path d="M166 526H928" className="diagram-path" markerEnd="url(#hidden-arrow)" />
        <path d="M692 552H928" className="diagram-path" markerEnd="url(#hidden-arrow)" />
        <text x="486" y="520" className="diagram-text">client leg to X (r1 {'->'} r2 {'->'} r3)</text>
        <text x="792" y="546" className="diagram-text">owner leg to X (r4 {'->'} r5 {'->'} r6)</text>

        <text x="72" y="602" className="diagram-note">Phase 5 encrypted data forwarding</text>
        <path d="M958 622H1186" className="diagram-path" markerEnd="url(#hidden-arrow)" />
        <text x="1036" y="614" className="diagram-text">X forwards tunnel stream</text>
        <text x="1146" y="642" className="diagram-text">to localhost target</text>

        <rect x="36" y="694" width="1268" height="96" rx="14" className="diagram-chip" />
        <text x="80" y="724" className="diagram-label">Crypto checkpoints</text>
        <text x="80" y="748" className="diagram-text">
          1) Descriptor encrypted before DHT storage.
        </text>
        <text x="80" y="770" className="diagram-text">
          2) Client verifies signature + expiry + consensus before setup.
        </text>
        <text x="650" y="748" className="diagram-text">
          3) Control messages include nonce/epoch replay protection.
        </text>
        <text x="650" y="770" className="diagram-text">
          4) Data path remains encrypted while owner app stays bound to localhost.
        </text>
      </svg>
    </DiagramFrame>
  );
